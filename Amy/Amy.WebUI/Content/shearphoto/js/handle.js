window.ShearPhoto.MINGGE(function () {
    ...
    Shear.config({
        /*---------------用户设置部份开始-----------------------------------------------------------------------*/
        relativeUrl: relativeUrl,  //取回相对路径，不懂原理的话
        ...
        /*HTML5重点功能*/	 HTML5Effects: false, //是否开启图片特效功能给用户  可选true false,  提示：有HTML5浏览器才会开启的！当然开启HTML5切图，该设置才有效
        ...
        preview: [100],// 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false
        url: "/user/upload/headimg",   //后端处理地址
        scopeWidth: 472,                 //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致
       ...
       */
       ...
       133                       //比例设置后，这个高度无效，由宽和比例来决定(整数型，禁止含小数点)  
        ...
        Min:80,           //截框拉伸或拖拽不能少于多少PX(整数型，禁止含小数点)  
        Max: 472,          //一开始启动时，图片的宽和高，有时候图片会很大的，必须要设置一下(整数型，禁止含小数点)，尽可能和scopeWidth值 一致  
    ...
    /*--------------------------------------------------------------截图成功后，返回来的callback-------------------------*/
    Shear.complete = function (data) {//截图成功完成时，由shearphoto.php返回数据过来的成功包
         //alert(data);//你可以调试一下这个返回包
        if(data.error!=0)
        {
            var msg = "发生错误，错误信息如下：" + (data.message ? data.message : "未知");
            layer.alert("" + msg , {
                btn: ['确定'], skin: "layui-layer-lan", yes: function (index) {
                    location.reload();
                }
            });
            return;
        ...
        {
			if(!!!$){ alert('JQuery库加载错误...'); }
			$(".iheadImg").each(function(i,o){				
				$(o).attr('src',data.url);
			});
        }
        ...
        layer.alert("恭喜你！头像上传成功", {
            btn: ['完成'], skin: "layui-layer-lan", yes: function (index) {
                this_.again();
                this_.pointhandle(3e3, 10, "上传完成！", 2, "#fbeb61", "#3a414c");
                layer.close(index);
            }
        });
    }
    ...
    ShearPhotoForm.imgFile.onclick = function () { return false }//一开始时先不让用户点免得事件阻塞
    var up = new ShearPhoto.frameUpImg({
        ...
        FilesSize: 1,                             //选择的图片不能超过 单位M（注意：是非HTML5时哦）
        ...
        preced: function (fun) { //点击选择图，载入图片时的事件
            ...
            document.getElementById("saveRecomm").style.display = "none";
            document.getElementById("saveShear").style.display = "block";

            var imgSelected = document.querySelectorAll(".recommend .selected");
            for (var i=0;i<imgSelected.length;i++ ) {
                imgSelected[i].className = imgSelected[i].className.replace("selected", "");
            }
    ...
    up.run(function (data, True) {
        //console.info(data);//你可以调试一下这个返回包
		...
		var resultUrl = data["success"] ||  data["url"];
        Shear.run(resultUrl, true);
    });
    /*.................................................选择图片上传的设置结束...............................................................*/
    ...
    Shear.addEvent(document.getElementById("saveShear"), "click", function () { //按下截图事件，提交到后端的shearphoto.php接收
        Shear.SendPHP();//我们示例截图并且传参数，后端文件shearphoto.php用 示例：$_POST["shearphoto"] 接收参数，不需要传参数请清空Shear.SendPHP里面的参数示例 Shear.SendPHP();
        //{ shearphoto: "我要传参数到服端", mingge: "我要传第二个参数到服务器" }
    });
    ...
    window.resetUploader = function (e,notip) {
        document.getElementById("saveRecomm").style.display = "block";
        document.getElementById("saveShear").style.display = "none";
        Shear.preview.close_();
        Shear.again();
        Shear.HTML5.EffectsReturn();
        Shear.HTML5.BOLBID && Shear.HTML5.URL.revokeObjectURL(Shear.HTML5.BOLBID);
        !notip && Shear.pointhandle(3e3, 10, "已取消！重新选择", 2, "#fbeb61", "#3a414c");
    }

    Shear.addEvent(document.getElementById("againIMG"), "click", function (e) {
        window.resetUploader.call(this, event);
    });
