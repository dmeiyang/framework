window.ShearPhoto.MINGGE(function () {
    ...
    Shear.config({
        ...
        preview: [100,60],// 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false
        url: "/user/upload/remote",   //后端处理地址
    ...
    Shear.complete = function (data) {//截图成功完成时，由shearphoto.php返回数据过来的成功包
        ...
        {
            var len = document.getElementsByClassName("zlhImg").length;
            for (var i = 0; i < len; i++)
            {
                document.getElementsByClassName("zlhImg")[i].src = data.url;
            }
            document.getElementById("uploadurl").value = data.url;
        }
        var this_ = this;
