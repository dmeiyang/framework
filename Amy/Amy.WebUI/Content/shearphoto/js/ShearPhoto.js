...
window.ShearPhoto.prototype = {
	...
	HTML5: {
		...
		CanvasImg: function(SendArry, postArgs, this_) {
            ...
            readerForm.append("imgFile", blob ,"uploadfile.jpg");
			return readerForm;
		}
	},
	...
	preview: {
		...
		run: function(arg, thisMain) {
			...
			if (Object.prototype.toString.call(arg.preview) === "[object Array]") {
			    var leng = arg.preview.length, EmptyFun = function () { }, srcDefault = "/images/no-portrait-min.png";
				if (leng > 0) {
					...
					//this.dom.parentNode.style.width = arg.scopeWidth + "px";
					...
					this.domimg = [domimg[0], leng],
                    this.close_ = function () {
						for (var i = 0; i < leng; i++) {
							domimgi = domimgA[i];
							domimgi.src = "/images/no-portrait-min.png";
	...
	run: function(ImgUrl, Trues, SetPro) {
		...
		var this_ = this,
			arg = this.arg,
			relatImgUrl = (this.HTML5.canvas && Trues) || ImgUrl.indexOf('http://') == 0 || ImgUrl.indexOf('/') == 0 ? ImgUrl : arg.relativeUrl + ImgUrl,
			image = new Image();
	...
	config: function(arg) {
		...
		scopeparentNode.style.width = scopeparentNode.parentNode.style.width = arg.scopeWidth  + "px";
		var opacityFun;
	...
	again: function() {
		...
		this.arg.ImgDom.src = "/images/no-portrait-min.png";
	},
	CoordinateData: function(True) {
		var SendPHPSmaller = function(W, H, P) {
			...
			SendArry = {};
		True || (SendArry.url = (this.ImgUrl.indexOf("http://") == 0 || this.ImgUrl.indexOf("/")==0 ? '' : "../") + this.ImgUrl);
		var R = {
				1: 270,
				2: 180,
				3: 90,
		...
		SendArry.ZM = (SendArry.FW / SendArry.IW).toFixed(6);
		return SendArry;
	},
	SendPHP: function(postArgs) {
		...
		this.SendUserMsg("正在为你处理截图，稍等...", 0, 2, "#fbeb61", "#3a414c", true, true, function() {
			if ((HTML5.HTML5PHP || HTML5.PhotoHTML5True) && HTML5.canvas) {
				...
				ResultData = "cropData=" + window.ShearPhoto.JsonString.JsonToString(cropInfo) + POSTHTML;
			}
			this_.MyAjax.carry({
				url: this_.arg.url,
				data: ResultData,
...
window.ShearPhoto.frameUpImg.prototype = {
	...
	DelCreateUpfile: function() {
		...
		inputfile.setAttribute("name", "imgFile");
